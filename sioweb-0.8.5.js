        ...
        sioweb = (function(){
            ...
            sioweb.merge({
                ...
                removeClass: function(val) {
                    ...
                    this['0'][0].className = className.replace(new RegExp('(\\s|^)'+val+'(\\s|$)'),' ').replace(/^\s+|\s+$/g, '');
                },
                get: function(val){
            ...
            sioweb.merge({
                uploadAjax: function(settings){
                    ...
                    events = this.merge({
                        defaultDrop: settings.defaultDrop||std_event
                    },events);
                    events = this.merge({
                        ...
                        drop: settings.drop||function(e){
                            ...
                            events.defaultDrop(e,files,settings);
                        }
                    },events);
                ...
                sendFiles: function(files,settings){
                    ...
                    settings.fileLabel = settings.fileLabel || 'files[]';
                    var form = this.FormData(),
                        selfObj = arguments[2]||sioweb,
                        selfItems = selfObj['0'],
                        url = location.search.length === 0 ? {} : JSON.parse('{"' + decodeURI(location.search.substring(1)).replace(/"/g, '\\"').replace(/&/g, '","').replace(/=/g,'":"') + '"}');
                    ...
                    for (var i = 0; i < files.length; i++)
                        form.append(settings.fileLabel, files[i]);

                    if(Object.keys(settings.input).length)
                        for(var input in settings.input)
                            form.append(input, settings.input[input]);
                    ...
                    this.ajax(selfObj.merge(settings,{
                        ...
                        files: files,
                        setAjaxHeader: true,
                        success: function(txt){console.log('success',txt)},
                        contentMimeType: form.contentMimeType||null,
                        url: settings.path?settings.path:'/',
                        data: form,
                    }));
                },
                FormData: function(){
